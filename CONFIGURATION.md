...
| `PUBLISH_BAD_TOPIC_SHAPE`       | Empty/wildcard segments           | Fix the topic shape                             |
| `PUBLISH_PLACEHOLDER_IATA`      | `XXX` placeholder                 | Run `get mqtt.iata` and configure the real code |
| `PUBLISH_TEST_INGRESS_DISABLED` | `test` not enabled                | Use a real IATA code                            |
| `PUBLISH_INVALID_IATA_FORMAT`   | Not 3 uppercase letters           | Run `get mqtt.iata`; uppercase the code         |
...
| `PUBLISH_BAD_PUBLIC_KEY`        | Topic key not 64 hex              | Fix the topic key                               |
