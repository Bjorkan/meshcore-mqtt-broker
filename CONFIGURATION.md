...
The heartbeat topic is exactly `heartbeat/` (with trailing slash). `heartbeat` without slash and `heartbeat/#` wildcards are denied. `mc-mqtt status` queries the live broker via `GET /status` and reports its identity, uptime, and observer count (or reports the broker is not running).
