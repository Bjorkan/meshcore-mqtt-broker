...
1. **Create a new application in Dokploy**
   ...
   - Set the application root/build path to `broker/`

2. **Configure environment variables**
   
   In the Dokploy environment variables section, add the values from `.env.example`, then adjust the key settings:
   ...
   SUBSCRIBER_2=viewer:another-password:2
...
1. **Clone the repository**
   ...
   cd meshcore-mqtt-broker/broker
   ```
...
3. **Create environment file**
   ...
   SUBSCRIBER_MAX_CONNECTIONS_DEFAULT=2
   SUBSCRIBER_1=admin:your-secure-password:1:10
   SUBSCRIBER_2=viewer:another-password:2
   ```
...
5. **Run as a service (optional but recommended)**
   ...
   Type=simple
   User=nodejs
   WorkingDirectory=/opt/meshcore-mqtt-broker/broker
...
See the broker README for client connection examples.
