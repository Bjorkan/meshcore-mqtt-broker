...
test('dashboard snapshot is built from Valkey reads, not local process fallback', async () => {
  ...
  state.recordPublish(publishPacket(`meshcore/GOT/${PUBLIC_KEY}/packets`), client);
  ...
  const store = {
    ...
    async getObserverClaims(publicKeys) {
      return new Map(publicKeys.map((publicKey) => [publicKey, 'Broker-VALKEY']));
    },
  ...
  assert.deepEqual(snapshot.brokers.map((broker) => broker.instanceId), ['Broker-VALKEY']);
  assert.deepEqual(snapshot.observers.map((observer) => observer.broker), ['Broker-VALKEY']);
