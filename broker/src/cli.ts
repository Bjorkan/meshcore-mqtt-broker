...
function readinessByInstance(readiness: ClusterInstanceReadiness[]): Map<string, ClusterInstanceReadiness> {
  return new Map(readiness.map((entry) => [entry.instanceId, entry]));
}

function statusRows(metrics: DashboardInstanceMetrics[], readiness: ClusterInstanceReadiness[], onlyInstance?: string): string[][] {
...
async function handleStatus(store: ClusterStateStore, options: CliOptions, cluster: boolean): Promise<void> {
  const [readiness, metrics] = await Promise.all([
    store.listInstanceReadiness(),
    store.listInstanceMetrics(),
  ]);
  const rows = statusRows(metrics, readiness, cluster ? undefined : options.instanceId);
  printTable(['Broker', 'Ready', 'Observers', 'Clients', 'Pub/min', 'Startad', 'Uppdaterad'], rows);
  if (!cluster && rows.length === 0) {
    console.log(`Ingen status hittades för instansen ${options.instanceId}.`);
  }
}

function observerRows(claims: Map<string, string>, observers: InstanceObserverEntry[], names: Map<string, string>, onlyInstance?: string): ObserverRow[] {
  const observersByKey = new Map(observers.map((observer) => [observer.publicKey.toUpperCase(), observer]));
  return Array.from(claims.entries())
    ...
    .sort((a, b) => a.owner.localeCompare(b.owner) || (a.label || a.publicKey).localeCompare(b.label || b.publicKey));
}

async function handleObserverList(store: ClusterStateStore, options: CliOptions, cluster: boolean): Promise<void> {
  ...
  const rows = observerRows(claims, observers, names, cluster ? undefined : options.instanceId)
    .map((observer) => [
  ...
  printTable(['Observer', 'Namn', 'Broker', 'Region', 'Senast', 'Meddelanden'], rows);
}
...
async function handleAbuseList(store: ClusterStateStore): Promise<void> {
  ...
  printTable(['Public key', 'Namn', 'Status', 'Orsak', 'Antal', 'Broker', 'Till'], rows);
}
...
async function confirmReset(namespace: string): Promise<boolean> {
  ...
  try {
    const answer = await rl.question(`Detta tömmer ALL Valkey-state i namespace "${namespace}". Fortsätt? [y/N] `);
...
async function handleReset(store: ClusterStateStore, options: CliOptions, confirm: (namespace: string) => Promise<boolean>): Promise<void> {
  ...
  console.log(`Valkey namespace "${options.namespace}" tömt: ${removed} nyckel${removed === 1 ? '' : 'ar'} borttag${removed === 1 ? 'en' : 'na'}.`);
}

export async function runCli(argv = process.argv.slice(2), dependencies: { confirmReset?: (namespace: string) => Promise<boolean> } = {}): Promise<number> {
  const args = [...argv];
  ...
  try {
    const [command, subcommand, value] = args.filter((arg) => !arg.startsWith('--'));
    ...
    if (command === 'reset' && !subcommand) {
      await handleReset(store, options, dependencies.confirmReset || confirmReset);
      return 0;
    }
...
function isEntrypoint(): boolean {
  return process.argv[1]?.endsWith('/cli.js') || process.argv[1]?.endsWith('/mc-mqtt');
}
