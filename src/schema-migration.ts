...
type Markers = {
  privateMarker: { schema_id: string; schema_version: string; schema_hash: string };
  publicMarker: { schema_id: string; schema_version: string; schema_hash: string };
};
...
async function tableCounts(client: pg.PoolClient): Promise<Record<string, string>> {
...
async function newIndexValidity(client: pg.PoolClient): Promise<Record<string, boolean>> {
  ...
  for (const name of [
    ...
    out[name] =
      result.rows.find((row) => row.name === name)?.valid === true;
  return out;
}
...
export async function emitMigrationSql(adminUrl: string): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      const actualV9 = await computeLegacyV9Fingerprint(
        client as unknown as Parameters<typeof computeLegacyV9Fingerprint>[0],
      );
      ...
      const v10 = await computeV10Fingerprint(
        client as unknown as Parameters<typeof computeV10Fingerprint>[0],
      );
      return [
...
export async function verifySchemaV10(
  ...
  try {
    ...
    try {
      ...
      const actual = await computeV10Fingerprint(
        client as unknown as Parameters<typeof computeV10Fingerprint>[0],
      );
      if (
...
export async function migrateSchemaV9ToV10(options: {
  ...
  try {
    ...
    try {
      ...
      if (version === 10) {
        const actual = await computeV10Fingerprint(
          workClient as unknown as Parameters<typeof computeV10Fingerprint>[0],
        );
        if (
          markers.privateMarker.schema_hash !== actual ||
          markers.publicMarker.schema_hash !== actual
      ...
      const actualV9 = await computeLegacyV9Fingerprint(
        workClient as unknown as Parameters<typeof computeLegacyV9Fingerprint>[0],
      );
      if (
      ...
      for (const table of COUNTED_TABLES)
        if (countsAfter[table] !== countsBefore[table])
          fail(`row count changed for ${table}: ${countsBefore[table]} -> ${countsAfter[table]}`);
