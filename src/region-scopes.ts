 ...
 *
 * Source: Statistiska centralbyrån (SCB), "Län och kommuner 2026".
 ...
 * known Swedish code. Scopes without a registered name use the scope code as
 * the name so every entry remains self-describing.
 */
export function regionScopeEntry(value: string): RegionScopeEntry {
  const scope = normalizeRegionScope(value);
  return { scope, name: regionScopeName(scope) ?? scope };
}
