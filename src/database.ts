...
CREATE INDEX IF NOT EXISTS public_packet_observations_region_received ON meshcore_public.packet_observations(region, received_at_ms DESC, id DESC);
...
CREATE INDEX IF NOT EXISTS public_node_sightings_region_received ON meshcore_public.node_sightings(region, received_at_ms DESC, id DESC);
...
CREATE TABLE IF NOT EXISTS meshcore_public.neighbor_snapshots (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, private_id bigint NOT NULL UNIQUE, observer_public_key text NOT NULL REFERENCES meshcore_public.observers(public_key) ON DELETE CASCADE, region text NOT NULL, reported_at_ms bigint, received_at_ms bigint NOT NULL, mqtt_retained boolean NOT NULL, self_scopes_json text NOT NULL, self_default_scope text, reported_total_neighbors integer CHECK (reported_total_neighbors >= 0), reported_queried_neighbors integer CHECK (reported_queried_neighbors >= 0), reported_truncated boolean, entry_count integer NOT NULL);
...
BEGIN
  INSERT INTO meshcore_public.neighbor_snapshots(private_id, observer_public_key, region, reported_at_ms, received_at_ms, mqtt_retained, self_scopes_json, self_default_scope, reported_total_neighbors, reported_queried_neighbors, reported_truncated, entry_count)
  SELECT NEW.id, o.public_key, NEW.region, NEW.reported_at_ms, NEW.received_at_ms, NEW.mqtt_retained, NEW.self_scopes_json, NEW.self_default_scope, NEW.reported_total_neighbors, NEW.reported_queried_neighbors, NEW.reported_truncated, NEW.entry_count FROM meshcore_public.observers o WHERE o.private_id = NEW.observer_id
  ON CONFLICT (private_id) DO UPDATE SET observer_public_key = EXCLUDED.observer_public_key, region = EXCLUDED.region, reported_at_ms = EXCLUDED.reported_at_ms, received_at_ms = EXCLUDED.received_at_ms, mqtt_retained = EXCLUDED.mqtt_retained, self_scopes_json = EXCLUDED.self_scopes_json, self_default_scope = EXCLUDED.self_default_scope, reported_total_neighbors = EXCLUDED.reported_total_neighbors, reported_queried_neighbors = EXCLUDED.reported_queried_neighbors, reported_truncated = EXCLUDED.reported_truncated, entry_count = EXCLUDED.entry_count;
  RETURN NEW;
END $$;
CREATE OR REPLACE FUNCTION meshcore_private.project_neighbor_entry() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO meshcore_public.neighbor_entries(private_id, snapshot_id, neighbor_public_key, snr, rssi, heard_secs_ago, calculated_last_heard_at_ms, status, scopes_json)
  SELECT NEW.id, s.id, NEW.neighbor_public_key, NEW.snr, NEW.rssi, NEW.heard_secs_ago, NEW.calculated_last_heard_at_ms, NEW.status, NEW.scopes_json FROM meshcore_public.neighbor_snapshots s WHERE s.private_id = NEW.snapshot_id
  ON CONFLICT (private_id) DO UPDATE SET snapshot_id = EXCLUDED.snapshot_id, neighbor_public_key = EXCLUDED.neighbor_public_key, snr = EXCLUDED.snr, rssi = EXCLUDED.rssi, heard_secs_ago = EXCLUDED.heard_secs_ago, calculated_last_heard_at_ms = EXCLUDED.calculated_last_heard_at_ms, status = EXCLUDED.status, scopes_json = EXCLUDED.scopes_json;
  RETURN NEW;
END $$;
