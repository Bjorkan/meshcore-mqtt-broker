...
/**
 * Current canonical schema version. Version 12 decouples the raw MQTT journal
 * from normalized history through compact provenance so raw retention cannot
 * cascade into current/public state.
 */
export const CURRENT_SCHEMA_VERSION = 12;
/** Only the current contract opens directly; versions 9-11 use the bounded migration chain. */
const ACCEPTED_SCHEMA_VERSIONS: readonly number[] = [12];
...
export const REQUIRED_OPERATIONAL_INDEXES = [
  {
    ...
    name: "mqtt_events_pending_claim",
    bootstrapSql:
      "CREATE INDEX IF NOT EXISTS mqtt_events_pending_claim ON meshcore_private.mqtt_events(id) WHERE processing_status = 'pending'",
    onlineSql:
      "CREATE INDEX CONCURRENTLY IF NOT EXISTS mqtt_events_pending_claim ON meshcore_private.mqtt_events(id) WHERE processing_status = 'pending'",
  },
  {
    schema: "meshcore_private",
    name: "mqtt_event_provenance_normalized_retention",
    bootstrapSql:
      "CREATE INDEX IF NOT EXISTS mqtt_event_provenance_normalized_retention ON meshcore_private.mqtt_event_provenance(received_at_ms, event_id) WHERE normalized_facts_present",
    onlineSql:
      "CREATE INDEX CONCURRENTLY IF NOT EXISTS mqtt_event_provenance_normalized_retention ON meshcore_private.mqtt_event_provenance(received_at_ms, event_id) WHERE normalized_facts_present",
  },
  {
    schema: "meshcore_private",
    name: "mqtt_events_recovery_scan",
    bootstrapSql:
      "CREATE INDEX IF NOT EXISTS mqtt_events_recovery_scan ON meshcore_private.mqtt_events(processing_started_at_ms, id) WHERE processing_status = 'processing'",
    onlineSql:
      "CREATE INDEX CONCURRENTLY IF NOT EXISTS mqtt_events_recovery_scan ON meshcore_private.mqtt_events(processing_started_at_ms, id) WHERE processing_status = 'processing'",
  },
] as const;
export const TIMESCALE_HYPERTABLES = [
  {
    schema: "meshcore_private",
    table: "observer_metrics",
    timeColumn: "received_at_ms",
    chunkIntervalMs: 604_800_000,
    bootstrapSql:
      "SELECT public.create_hypertable('meshcore_private.observer_metrics'::regclass, public.by_range('received_at_ms'::name, 604800000::bigint), if_not_exists => TRUE)",
    migrationSql:
      "SELECT public.create_hypertable('meshcore_private.observer_metrics'::regclass, public.by_range('received_at_ms'::name, 604800000::bigint), migrate_data => TRUE, if_not_exists => TRUE)",
  },
] as const;

export const PUBLIC_OBSERVER_METRICS_VIEW_SQL = `CREATE OR REPLACE VIEW meshcore_public.observer_metrics AS
SELECT COALESCE(legacy.public_id, CASE WHEN metric.id <= state.legacy_private_max THEN metric.id ELSE metric.id + state.new_id_offset END) AS id, metric.id AS private_id, observer.public_key AS observer_public_key, metric.received_at_ms, metric.reported_at_ms, metric.metric_name, metric.numeric_value, metric.text_value, metric.boolean_value, metric.unit
FROM meshcore_private.observer_metrics metric
CROSS JOIN meshcore_private.observer_metric_public_id_state state
LEFT JOIN meshcore_private.observer_metric_public_ids legacy ON legacy.private_id = metric.id
JOIN meshcore_public.observers observer ON observer.private_id = metric.observer_id
...
const PRIVATE_TABLES = [
  ...
  "observers",
  "mqtt_events",
  "mqtt_event_provenance",
  "observer_metric_public_ids",
  "observer_metric_public_id_state",
...
// Exported: scripts/sync-schema-asset.ts renders the static initdb file from
// these constants so database.ts is the single source of truth.
export const PRIVATE_SCHEMA_DDL = `
...
CREATE TABLE IF NOT EXISTS meshcore_private.mqtt_event_provenance (event_id bigint PRIMARY KEY, topic text NOT NULL, iata text CHECK (iata IS NULL OR iata ~ '^[A-Z]{3}$'), observer_public_key text CHECK (observer_public_key IS NULL OR length(observer_public_key) = 64), subtopic text, subtopic_root text, payload_sha256 text NOT NULL, payload_size_bytes integer NOT NULL CHECK (payload_size_bytes >= 0), qos integer NOT NULL CHECK (qos BETWEEN 0 AND 2), retain boolean NOT NULL, dup boolean NOT NULL, received_at_ms bigint NOT NULL, parser_name text NOT NULL, parser_version text NOT NULL, collector_instance_id text NOT NULL, normalized_facts_present boolean NOT NULL DEFAULT false, created_at_ms bigint NOT NULL);
CREATE INDEX IF NOT EXISTS mqtt_event_provenance_received ON meshcore_private.mqtt_event_provenance(received_at_ms, event_id);
CREATE INDEX IF NOT EXISTS mqtt_event_provenance_observer_iata_received ON meshcore_private.mqtt_event_provenance(observer_public_key, iata, received_at_ms, event_id) WHERE observer_public_key IS NOT NULL AND iata IS NOT NULL;
${REQUIRED_OPERATIONAL_INDEXES.map((index) => `${index.bootstrapSql};`).join("\n")}
-- Existing v11 public metric cursors are preserved here during migration. Fresh v12 databases keep this map empty.
CREATE TABLE IF NOT EXISTS meshcore_private.observer_metric_public_ids (private_id bigint PRIMARY KEY, public_id bigint NOT NULL UNIQUE);
CREATE TABLE IF NOT EXISTS meshcore_private.observer_metric_public_id_state (singleton integer PRIMARY KEY CHECK (singleton = 1), legacy_private_max bigint NOT NULL CHECK (legacy_private_max >= 0), new_id_offset bigint NOT NULL CHECK (new_id_offset >= 0));
INSERT INTO meshcore_private.observer_metric_public_id_state(singleton, legacy_private_max, new_id_offset) VALUES (1, 0, 0) ON CONFLICT (singleton) DO NOTHING;
...
CREATE TABLE IF NOT EXISTS meshcore_private.observer_status_events (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, mqtt_event_id bigint NOT NULL UNIQUE REFERENCES meshcore_private.mqtt_event_provenance(event_id) ON DELETE RESTRICT, observer_id bigint NOT NULL REFERENCES meshcore_private.observers(id) ON DELETE CASCADE, iata text NOT NULL CHECK (iata ~ '^[A-Z]{3}$'), reported_at_ms bigint, received_at_ms bigint NOT NULL, origin text, model text, firmware_version text, raw_json text NOT NULL, created_at_ms bigint NOT NULL);
CREATE TABLE IF NOT EXISTS meshcore_private.observer_metrics (id bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL, observer_id bigint NOT NULL REFERENCES meshcore_private.observers(id) ON DELETE CASCADE, mqtt_event_id bigint NOT NULL REFERENCES meshcore_private.mqtt_event_provenance(event_id) ON DELETE RESTRICT, received_at_ms bigint NOT NULL, reported_at_ms bigint, metric_name text NOT NULL, numeric_value double precision, text_value text, boolean_value boolean, unit text, CHECK ((numeric_value IS NOT NULL)::int + (text_value IS NOT NULL)::int + (boolean_value IS NOT NULL)::int = 1), PRIMARY KEY(id, received_at_ms), UNIQUE(mqtt_event_id, metric_name, received_at_ms));
${TIMESCALE_HYPERTABLES.map((table) => `${table.bootstrapSql};`).join("\n")}
CREATE INDEX IF NOT EXISTS observer_metrics_observer_metric_received ON meshcore_private.observer_metrics(observer_id, metric_name, received_at_ms DESC, id DESC);
CREATE TABLE IF NOT EXISTS meshcore_private.observer_radio_history (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, observer_id bigint NOT NULL REFERENCES meshcore_private.observers(id) ON DELETE CASCADE, mqtt_event_id bigint NOT NULL UNIQUE REFERENCES meshcore_private.mqtt_event_provenance(event_id) ON DELETE RESTRICT, frequency_mhz double precision, bandwidth_khz double precision, spreading_factor integer, coding_rate integer, tx_power_dbm double precision, received_at_ms bigint NOT NULL);
...
CREATE INDEX IF NOT EXISTS neighbor_snapshots_observer_received ON meshcore_private.neighbor_snapshots(observer_id, id DESC);
...
CREATE TABLE IF NOT EXISTS meshcore_private.packet_observations (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, packet_id bigint NOT NULL REFERENCES meshcore_private.packets(id) ON DELETE CASCADE, mqtt_event_id bigint NOT NULL UNIQUE REFERENCES meshcore_private.mqtt_event_provenance(event_id) ON DELETE RESTRICT, observer_id bigint NOT NULL REFERENCES meshcore_private.observers(id) ON DELETE CASCADE, iata text NOT NULL CHECK (iata ~ '^[A-Z]{3}$'), received_at_ms bigint NOT NULL, reported_at_ms bigint, rssi double precision, snr double precision, score double precision, direction text, suspected_mqtt_duplicate boolean NOT NULL DEFAULT false, suspected_rf_retransmission boolean NOT NULL DEFAULT false, created_at_ms bigint NOT NULL);
CREATE INDEX IF NOT EXISTS packet_observations_packet_observer_received ON meshcore_private.packet_observations(packet_id, observer_id, received_at_ms DESC, id DESC);
...
CREATE TABLE IF NOT EXISTS meshcore_private.processing_errors (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, mqtt_event_id bigint NOT NULL REFERENCES meshcore_private.mqtt_event_provenance(event_id) ON DELETE RESTRICT, packet_id bigint REFERENCES meshcore_private.packets(id) ON DELETE SET NULL, stage text NOT NULL, error_code text NOT NULL, error_message text NOT NULL, processor_name text NOT NULL, processor_version text NOT NULL, received_at_ms bigint NOT NULL, created_at_ms bigint NOT NULL, UNIQUE(mqtt_event_id, stage, error_code, processor_version));
`;
...
export const PUBLIC_PROJECTION_DDL = `
...
export const PUBLIC_PROJECTION_TRIGGERS_DDL = `
 ...
 */
export const V10_PUBLIC_INDEX_DDL = `
...
export class ApplicationDatabase implements ApplicationTransaction {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      const fingerprint = await computeCurrentSchemaFingerprint(client);
      await client.unsafe(
  ...
  private async validateCurrentSchema(): Promise<void> {
    ...
    // Only the current contract opens directly; known v9-v11 layouts get one
    ...
    if (!ACCEPTED_SCHEMA_VERSIONS.includes(actualVersion))
      throw new IncompatibleDatabaseError(
        `schema-version ${actualVersion} stöds inte; förväntas ${ACCEPTED_SCHEMA_VERSIONS.join(
          " eller ",
        )}. Kör bun run db:migrate för en känd äldre databas.`,
        {
    ...
    try {
      ...
      const fingerprint =
        actualVersion === 10
          ...
          : await computeCurrentSchemaFingerprint(client);
...
async function seedRegionScopeRegistry(client: SqlExecutor): Promise<void> {
  ...
  await execute(
    `INSERT INTO meshcore_public.region_scopes(region, name, manually_added)
     ...
     WHERE (meshcore_public.region_scopes.name, meshcore_public.region_scopes.manually_added)
       IS DISTINCT FROM (EXCLUDED.name, TRUE)`,
    client,
    values,
  );
}
...
export async function computeV11Fingerprint(
  ...
  return computePublicSchemaFingerprintFormat(client, {
    header: `schema|${SCHEMA_ID}|11|${FINGERPRINT_FORMAT_V2}`,
    includeIndexes: false,
  });
}

/** Version 12 changes private storage/retention while preserving public columns. */
export async function computeV12Fingerprint(
  client: SqlExecutor,
): Promise<string> {
  return computePublicSchemaFingerprintFormat(client, {
    header: `schema|${SCHEMA_ID}|12|${FINGERPRINT_FORMAT_V2}`,
    includeIndexes: false,
  });
}

export const computeCurrentSchemaFingerprint = computeV12Fingerprint;
