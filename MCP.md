...
| `search_messages`             | Page logical messages (default) or one row per raw packet with `observation_count`; filters include encryption, signature validity, region, and observer |
...
- **Snapshot watermark.** The first page of a time-ordered query freezes both effective bounds (`effective_from` and `effective_to`) inside the cursor. Later pages query the same logical result set even while new traffic ingests; no server-side snapshot is stored. This applies to all paginated tools, including `search_path_prefixes` where `occurrence_count` and `last_seen_at` otherwise change under live ingest. Tools that sort on mutable keys (`list_nodes`, `list_observers` by `last_seen_at`) additionally freeze an `as_of` snapshot time in the cursor, and their sort values are computed as of that time on every page. `search_paths` and `search_path_prefixes` freeze a `resolution_as_of` in the cursor so hop-prefix resolution uses only node knowledge known by the first page; a brand-new query gets a fresh resolution snapshot.
- **Cursor size.** The transport schemas accept cursors up to 4,096 characters; the server guarantees every cursor it generates stays within that bound for the maximally filled filter sets.
...
- **Retention semantics.** Both the lower and upper effective bounds (`effective_from` and `effective_to`) are immutable after the first page, including when the client did not send `from`/`to` explicitly; continuation pages reuse the cursor values exactly and never recompute default windows from current server time. A cursor whose frozen window lies entirely outside the retained history is rejected with `cursor_outside_retention_window` instead of silently returning inconsistent pages.
- **Required first-page arguments.** Endpoints with required identity or window arguments (`get_observer_status_history`, `get_neighbor_history`, `get_node_adverts`, `get_node_sightings`, `get_node_position_history`, `get_signal_history`, `get_activity_timeseries`) require them on the first page but accept continuation pages with only `cursor` and `limit`; the verified cursor payload restores the canonical arguments.
