...
- `src/channel-key-registry.ts`: opt-in channel decryption key store built from `config.yaml` (explicit PSKs and hashtag-derived keys) with hash-to-channel-name resolution; keys never enter the database.
...
The MCP V2 route and the Fastify REST API are unauthenticated and read-only and share one public-history contract documented in [`MCP.md`](MCP.md) and [`REST_API.md`](REST_API.md). REST routes, MCP tools, and the schema dictionary are bound to the same query service, DTO schemas, and public-output policy; there is no second implementation of query semantics. The path tools (`search_paths`, `search_path_prefixes`) resolve hop prefixes at query time against `node_prefix_candidates` so historical paths resolve against later-known nodes; the event stream (`search_events`) unions bounded time-windowed branch scans across six event tables and paginates by `(timestamp, event_type, event_id)` without server-side client state. Each request reads only explicit normalized history through bound SQL and passes the complete result through one recursive policy before serialization. Request bodies, concurrent requests, query windows, page sizes, cursor shape, bucket counts, output traversal, and final serialized size are bounded. Graceful shutdown closes the MCP HTTP handler and the Fastify app before the shared database connection.

The query surface is fully stateless: no client, session, watch, subscription, watermark, or cursor state exists anywhere. Pagination cursors are self-contained (version, query identity, canonical filters, sort/order, keyset position, tie-breakers, and a frozen effective time window) and HMAC-signed with a per-database secret stored in `cursor_signing_secret`, so cursors survive restarts, work across instances sharing the database, and cannot be forged. Live ingest cannot change an in-flight result set because the first page freezes `effective_to` in the cursor; keyset pagination (never offset) keeps pages stable and replayable.
...
Per-publisher trust state is hydrated from `trust_state` once per process per key (first publish after start or eviction) and then lives in memory as the authoritative copy. It is persisted at most once per 60-second interval per key, immediately on mute/allow status changes, and on publisher disconnect when the interval or status-change condition is met; persistence failures are logged and retried no faster than the interval. In-memory trust state for publishers inactive more than 30 days is evicted while the durable row remains subject to its own 90-day expiration, so a returning publisher is rehydrated instead of accumulating state forever. Identical denial events from one key and reason are recorded at most once per 30 seconds; expired denial rows are drained by the periodic bounded cleanup.
