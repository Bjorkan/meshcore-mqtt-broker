...
MQTT WebSocket upgrades, the MCP handler, the public-tool HTTP adapter, the API handler, and the dashboard handler share one HTTP listener and port. The WebSocket server handles upgrades and passes accepted streams to Aedes. `createWebServer()` dispatches the exact MCP route and `/api/v2/tools/{toolName}` before its ordinary GET/HEAD gate, then runs discovery/documentation/dashboard API routing before the dashboard static handler and owns 404/405/503 fallbacks. It applies 30-second request, 15-second header, and 5-second idle keep-alive limits plus `nosniff` and strict-origin referrer headers; upgraded MQTT WebSocket streams are not HTTP keep-alive requests. The public tool adapter accepts only JSON `POST`; other API/dashboard resources remain GET/HEAD. The dashboard browser code fetches `/api/dashboard` over HTTP. Its MapLibre 6 ESM bundle uses a separately built, same-origin worker served at `/maplibre-gl-worker.js`; both remain container-local assets while map tiles are optional browser-side outbound requests. The unauthenticated dashboard snapshot remains a separate operational compatibility surface. `/api/v1` is removed and returns `410`; public integrations use the normalized V2 query registry.

The MCP V2 route and plain HTTP tool mirror are unauthenticated and read-only and share one public-history contract documented in [`MCP.md`](MCP.md). Registration binds each handler, strict input/output schema, query, and public-output policy to MCP and to `POST /api/v2/tools/{toolName}`; there is no second implementation of query semantics. Each request reads only explicit normalized history through bound SQL, passes the complete result through one recursive policy, and validates successful structured content against the registered output schema before HTTP serialization. Request bodies, concurrent requests, query windows, page sizes, cursor shape, bucket counts, output traversal, and final serialized size are bounded. Graceful shutdown closes the MCP HTTP handler before the shared database connection.

`/api/openapi.json` is the source-controlled OpenAPI 3.1 contract. `/api/docs` loads an explicit allowlist of local Swagger UI distribution assets from the installed runtime dependency, with same-origin references, a restrictive content-security policy, read-only GET/POST “Try it out,” no CDN, and no remote validator. API documentation does not create another process, port, or outbound runtime dependency.
