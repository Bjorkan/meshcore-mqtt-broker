...
| Method   | Path                                   | Purpose                                                     |
| -------- | -------------------------------------- | ----------------------------------------------------------- |
| GET/HEAD | `/`                                    | HTML shell with dashboard bootstrap                         |
...
| GET/HEAD | `/dashboard-client.css`                | Bundled Astryx component and theme styles                   |

**Key types:**

- `DashboardState`: In-memory tracking of connected clients/observers
- `DashboardSnapshot`: generatedAt, respondingBroker, summary, brokers, observers, bans, subscribers, and MeshCore.io state
...
- `renderDashboardHtml()`: Generates the HTML shell for the bundled dashboard client
- `createDashboardServer()`: Creates and returns the HTTP server

**CSS:** Astryx reset, component styles, and the pre-built local Astryx theme are bundled by esbuild into `/dashboard-client.css`. The dashboard has no application-specific stylesheet; visual color, typography, spacing, radius, layout, responsiveness, and component behavior come from the Astryx theme and component props. Theme source lives in `src/themes/gothic/gothicTheme.ts`; `gothicTheme.css`, `gothic.js`, and `gothic.d.ts` are generated by `npx astryx theme build`.
...
Browser-side React application bundled with esbuild. Uses hash-based routing (`#overview`, `#brokers`, `#observers`, `#meshcoreio`, `#bans`, `#subscribers`). Fetches data every 5 seconds from `/api/dashboard`. No external state management library; state uses React hooks.

The UI is an Astryx-only operations console. `AppShell`, `TopNav`, and `SideNav` own the responsive frame and mobile navigation. The local Astryx Gothic theme is dark-only and owns color, typography, spacing, radius, icons, and component styling. KPI widgets use cards; scannable operational records remain edge-to-edge Astryx tables or lists. Third-party UI frameworks and application-specific CSS are intentionally excluded. MeshCore.io position data is shown as an Astryx list rather than an interactive third-party map.

**Views:**

- **Overview**: Cluster KPI cards, public observer lookup, broker health and distribution, compact MeshCore.io state, protection events, and recent publishes
- **Brokers**: Broker table with sortable columns, broker modal showing per-broker observers
- **Observers**: Search and region filtering, sortable observer table, observer detail with protection state, neighbors, and messages
- **MeshCore.io**: Queue and worker health, recent positioned adverts, broker workers, and upload history
- **Bans**: Denied publishes and trust-state events with detail dialogs
- **Subscribers**: Active subscriber accounts, topic filters, broker distribution, and connection details

**Key components:**

- `RegionDisplay` → renders county name + IATA code via `formatRegionDisplay` helper
- `PublishFeed` → compact six-column Astryx table with bounded expansion
- `ObserverModal` / `BrokerModal` / `BanModal` / `SubscriberModal` → Astryx dialogs with focus management and scroll locking
- `ObserverSearch` → Astryx text input and region selector
- `ObserverLookup` → public key lookup form calling `/api/v1/observers/.../status`
- `StatusLabel` → visible status text paired with an Astryx `StatusDot`
- `Panel`, `MetricItem`, `Empty`, `Icon` → local compositions built from Astryx primitives
