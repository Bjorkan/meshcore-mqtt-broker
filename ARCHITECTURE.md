...
`GET /api/v1/nodes` reads only nodes heard during the rolling last seven days. Its collection representation contains lightweight node summaries and all active region codes, but omits raw packets and per-observer hearing rows. A normal region filter matches membership in that active set. A region disappears after seven days without a receipt from that region even if other regions continue to hear the node. Reads are deterministically bounded to 10,000 selected nodes and 100,000 joined region rows. The node-detail route performs a bound-parameter lookup limited to 10,000 active region rows, returns no expired advert, and adds the retained raw packet plus detailed active region hearings. `region=SWE` instead applies a local point-in-multipolygon test to advert coordinates using the bundled Natural Earth 1:10m Admin 0 Sweden geometry; it excludes adverts without coordinates. Region summaries aggregate only this bounded active node view and receive display/primary mappings synchronously from `RegionRegistry`; they expose no whitelist decision. The database stores no derived country classification, and API reads perform no network I/O.
