...
The public reader contract now includes schema metadata, keyset-pagination indexes, relational neighbor-scope tables, PostGIS node/advert locations, and region-leading hearing indexes for IATA-region filtering. Existing deployments must be reprovisioned according to the clean-install schema lifecycle (schema version 4) before a read-only HTTP/MCP process uses these fields.
