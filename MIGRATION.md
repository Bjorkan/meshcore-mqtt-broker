...
Schema v10 introduced fingerprint-v2 and timeline indexes. Schema v11 adds persisted `database_created_at`. Schema v12 adds compact `mqtt_event_provenance`, rewires normalized fact foreign keys away from raw `mqtt_events`, and replaces the physical public observer-metrics projection with a view over the canonical private metric store. Startup has one known chain, 9→10→11→12, under one deadline. The v11→v12 step is transactional. Before replacing the physical public metric table, it records every legacy `(private_id, id)` cursor pair in a compact private mapping and stores an offset for post-cutover IDs. This preserves existing public metric cursors even when the old public identity sequence had diverged from the private one, without retaining the duplicate metric payload table or a per-row projection trigger. Automatic startup still follows the repository's availability-first rule and may reprovision after a reachable migration failure, so operators who need guaranteed history preservation should run the manual migration after taking a backup. The manual `bun run db:migrate` command uses the same registry and never resets implicitly.

Fresh v12 bootstrap creates `meshcore_private.observer_metrics` as a Timescale hypertable directly. The semantic `bun run db:migrate` path deliberately does not perform a potentially long physical rewrite. For an existing v12 database, take a backup, stop or quiesce metric publishers, set `DATABASE_TIMESCALE_OPTIMIZATION_TIMEOUT_MS`, and run `DATABASE_URL=... bun run db:optimize-timescale`; it validates the v12 fingerprint, changes the private uniqueness keys to include `received_at_ms`, and calls `create_hypertable(..., by_range(...), migrate_data => TRUE)`. Validate row counts, projection counts, the seven-day dimension in `timescaledb_information.dimensions`, and normal REST integration before resuming writes. Rollback is by restoring the backup, or during the same maintenance window by copying rows into a regular replacement table with the former keys, recreating projection triggers, validating counts, and swapping tables. Do not attempt an in-place extension catalog edit.
