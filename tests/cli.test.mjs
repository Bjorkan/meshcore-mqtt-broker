...
function publicKey(prefix) {
  return `${prefix}${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
    0,
    64,
  );
}
...
async function configForInstance(namespace, instanceId) {
  const tempDir = await mkdtemp(
    path.join(os.tmpdir(), "meshcore-cli-id-test-"),
  );
...
test("mc-mqtt reset requires confirmation and clears the Valkey namespace", async () => {
  ...
  const cancelled = await captureCli(["reset"], config, {
  ...
  const reset = await captureCli(["reset"], config, {
    confirmReset: async (confirmedNamespace) => {
