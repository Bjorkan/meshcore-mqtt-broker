...
test("package remains an ESM broker package", async () => {
  ...
  assert.match(
    pkg.scripts.build,
    /^tsc && esbuild src\/dashboard-client\.tsx /,
  );
  assert.equal(
    pkg.scripts.test,
  ...
  assert.equal(pkg.devDependencies["maplibre-gl"], "5.24.0");
...
test("Jest runs ESM integration tests without TypeScript transforms", async () => {
  const config = (await import(path.join(projectDir, "jest.config.mjs")))
    .default;
...
test("Dockerfile builds and runs on the configured Node major", async () => {
  const nodeVersion = (
    await readFile(path.join(projectDir, ".node-version"), "utf8")
  ).trim();
  ...
  assert.match(
    dockerfile,
    /^HEALTHCHECK --interval=45s --timeout=50s --start-period=20s --retries=3 CMD \["node", "dist\/healthcheck\.js"\]$/m,
  );
  assert.match(
    dockerfile,
    /chmod \+x \/usr\/local\/bin\/docker-entrypoint\.sh \/app\/dist\/cli\.js/,
  );
  assert.match(
    dockerfile,
...
test("Docker healthcheck verifies both MQTT loopback and Valkey readiness", async () => {
  const healthcheckSource = await readFile(
    path.join(projectDir, "src/healthcheck.ts"),
    "utf8",
  );
...
test("Docker runtime image removes bundled npm CVE surface", async () => {
  ...
  assert.ok(
    dockerfile.indexOf("rm -rf /usr/local/lib/node_modules/npm") >
...
test("Docker image drops root privileges without requiring a local data volume", async () => {
  ...
  const entrypoint = await readFile(
    path.join(projectDir, "docker-entrypoint.sh"),
    "utf8",
  );
...
test("broker workflow publishes to Docker Hub and GitHub Packages", async () => {
  ...
  assert.match(
    workflow,
    /ghcr\.io\/bjorkan\/meshcore-mqtt-broker:sha-\$\{SHORT_SHA\}/,
  );
});
...
test("broker workflow gates Docker publishing on broker tests", async () => {
  ...
  assert.ok(
    ...
    "testjobbet ska definieras före build",
  );
  assert.ok(
    workflow.indexOf("build:") < workflow.indexOf("publish:"),
    "buildjobbet ska definieras före publish",
  );
});
...
test("broker Valkey runtime uses Valkey 9 in examples and CI", async () => {
  ...
  const composeExample = await readFile(
    path.join(repoDir, "compose.yaml.example"),
    "utf8",
  );
...
test("runtime logs do not use legacy English log categories", async () => {
  const runtimeSources = await Promise.all(
    ["src/server.ts", "src/abuse-detector.ts", "src/rate-limiter.ts"].map(
      (filePath) => readFile(path.join(projectDir, filePath), "utf8"),
    ),
  );
...
test("Valkey Aedes adapters attach error listeners", async () => {
  const orchestrationSource = await readFile(
    path.join(projectDir, "src/orchestration.ts"),
    "utf8",
  );
...
test("Valkey runtime writes are configured with TTLs", async () => {
  ...
  assert.match(
    orchestrationSource,
    /TRUST_STATE_TTL_MS = 90 \* 24 \* 60 \* 60 \* 1000/,
  );
  assert.match(
    orchestrationSource,
  ...
  assert.match(
    orchestrationSource,
    /packetTTL\(\) \{\s*return AEDES_PACKET_TTL_SECONDS;\s*\}/,
  );
});
...
test("broker workflow scans the built image with Docker Scout before upload", async () => {
  ...
  assert.ok(
    workflow.indexOf("Docker Scout broker image") >
      workflow.indexOf("Build broker image"),
    "Docker Scout should run after the broker image build",
  );
  assert.ok(
    workflow.indexOf("Docker Scout broker image") <
      workflow.indexOf("Upload broker image artifact"),
