...
test("claimObserver sets initial claim and returns null on first call", async () => {
  ...
  const pk =
    `A${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("ready rejects duplicate broker instance IDs that are already registered in Valkey", async () => {
  ...
  await first.ready();
  await assert.rejects(second.ready(), DuplicateBrokerInstanceIdError);
});
...
test("claimObserver returns null when same instance reclaims", async () => {
  ...
  const store = createStore("broker-alpha", ns);
  const pk =
    `B${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("claimObserver returns previous owner when another instance takes over", async () => {
  ...
  const pk =
    `C${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("renewObserverClaim returns true when claim is still owned", async () => {
  ...
  const store = createStore("broker-alpha", ns);
  const pk =
    `D${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("renewObserverClaim returns false when claim does not exist", async () => {
  ...
  const store = createStore("broker-alpha", ns);
  const pk =
    `E${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("renewObserverClaim returns false when claim was taken by another instance", async () => {
  ...
  const beta = createStore("broker-beta", ns);
  const pk =
    `F${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("renewObserverClaim refreshes TTL on owned claim", async () => {
  ...
  const pk =
    `G${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
  ...
  try {
    ...
    assert.ok(
      ttlAfter > 0,
      `expected positive TTL after renew, got ${ttlAfter}`,
    );
  } finally {
    await redis.quit();
  }
...
test("getObserverClaim returns correct owner", async () => {
  ...
  const beta = createStore("broker-beta", ns);
  const pk =
    `H${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("getObserverClaim returns null for unclaimed observer", async () => {
  ...
  const store = createStore("broker-alpha", ns);
  const pk =
    `I${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("claims are independent per public key", async () => {
  ...
  const beta = createStore("broker-beta", ns);
  const pk1 =
    `J1${uniqueId().replace(/-/g, "").toUpperCase().padEnd(62, "F")}`.slice(
      0,
      64,
    );
  const pk2 =
    `J2${uniqueId().replace(/-/g, "").toUpperCase().padEnd(62, "F")}`.slice(
      0,
      64,
    );
...
test("claims are isolated by Valkey namespace", async () => {
  ...
  const pk =
    `K${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("releaseObserverClaimsForInstance releases every local claim and leaves other brokers alone", async () => {
  ...
  const redis = new Redis(kvUrl(), { maxRetriesPerRequest: 1 });
  const pk1 =
    `O1${uniqueId().replace(/-/g, "").toUpperCase().padEnd(62, "F")}`.slice(
      0,
      64,
    );
  const pk2 =
    `O2${uniqueId().replace(/-/g, "").toUpperCase().padEnd(62, "F")}`.slice(
      0,
      64,
    );
  const pk3 =
    `O3${uniqueId().replace(/-/g, "").toUpperCase().padEnd(62, "F")}`.slice(
      0,
      64,
    );
  ...
  try {
    ...
    await alpha.acceptObserverStatusTimestamp(
      pk1,
    ...
    assert.equal(
      await redis.get(`${ns}:observers:${pk1}:status-timestamp`),
      null,
    );
...
test("instance metrics persist uplink counters in Valkey", async () => {
  ...
  const uplinkMetrics = metrics.find(
    (entry) => entry.instanceId === "broker-uplink",
  );
...
test("close clears this instance readiness, metrics, observers, subscriber registrations, and claims", async () => {
  ...
  try {
    ...
    await store.acceptObserverStatusTimestamp(
      pk,
      Date.parse("2026-01-02T00:00:00.000Z"),
      60_000,
    );
    ...
    assert.equal(
      await redis.get(`${ns}:instances:broker-closing:metrics`),
      null,
    );
    assert.equal(
      await redis.get(`${ns}:instances:broker-closing:observers`),
      null,
    );
    assert.deepEqual(
      await redis.zrange(`${ns}:subscribers:viewer:connections`, 0, -1),
      [],
    );
    assert.equal(await redis.get(`${ns}:observers:${pk}:claim`), null);
    assert.equal(await redis.get(`${ns}:observers:${pk}:node-name`), null);
    assert.equal(
      await redis.get(`${ns}:observers:${pk}:status-timestamp`),
      null,
    );
...
test("concurrent claims from multiple instances resolve to a single winner", async () => {
  ...
  const pk =
    `L${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
  ...
  assert.equal(
    nullCount,
    1,
    `expected exactly one first claim, got ${nullCount}`,
  );
  ...
  for (const r of results) {
    if (r !== null) {
      assert.ok(
        ...
        `expected previous owner string, got ${r}`,
      );
    }
  }
  ...
  const renewResults = await Promise.all(
    instances.map((s) => s.renewObserverClaim(pk)),
  );
  const ownerCount = renewResults.filter((r) => r === true).length;
  assert.equal(
    ownerCount,
    1,
    `expected exactly one claim owner, got ${ownerCount}`,
  );
});
...
test("expired claim can be reclaimed by any instance", async () => {
  ...
  const redis = new Redis(kvUrl(), { maxRetriesPerRequest: 1 });
  const pk =
    `M${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
...
test("full lifecycle: claim -> renew -> takeover -> reclaim after expiry", async () => {
  ...
  const redis = new Redis(kvUrl(), { maxRetriesPerRequest: 1 });
  const pk =
    `N${uniqueId().replace(/-/g, "").toUpperCase().padEnd(64, "F")}`.slice(
      0,
      64,
    );
