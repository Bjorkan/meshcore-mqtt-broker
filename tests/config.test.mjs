  ...
  return {
    ...
    subscribers: {
      ...
      users: [
        ...
        { username: 'admin', password: 'admin-pass', role: 1, max_connections: 10 },
      ],
...
function deletePath(object, path) {
  const parent = path.slice(0, -1).reduce((current, part) => current?.[part], object);
...
test('loads runtime settings, subscribers, and allowed region metadata from config.yaml shape', () => {
  withConfig(baseConfig({
    ...
    broker: { kv_url: 'redis://yaml-valkey:6379', kv_namespace: 'yaml-namespace' },
    ...
    assert.deepEqual(subscriberConfig.users.map((user) => user.username), ['viewer', 'admin']);
...
test.each([
  ['mqtt.ws_port rejects non-numeric values', loadMqttConfig, ['mqtt', 'ws_port'], 'abc', /mqtt\.ws_port/, /heltal/],
  ['mqtt.ws_port rejects ports above the TCP range', loadMqttConfig, ['mqtt', 'ws_port'], '65536', /mqtt\.ws_port/, /högst 65535/],
  ['auth.expected_audience is required', loadMqttConfig, ['auth', 'expected_audience'], undefined, /auth\.expected_audience/, /saknas/],
  ['auth.expected_audience rejects whitespace-only values', loadMqttConfig, ['auth', 'expected_audience'], '   ', /auth\.expected_audience/, /mellanslag/],
  ['abuse.bucket_refill_rate rejects non-numeric values', loadAbuseConfig, ['abuse', 'bucket_refill_rate'], 'foo', /abuse\.bucket_refill_rate/, /giltigt tal/],
  ['abuse.bucket_refill_rate rejects non-positive values', loadAbuseConfig, ['abuse', 'bucket_refill_rate'], '0', /abuse\.bucket_refill_rate/, /större än 0/],
  ['abuse.enforcement_enabled rejects non-boolean values', loadAbuseConfig, ['abuse', 'enforcement_enabled'], 'yes', /abuse\.enforcement_enabled/, /true/],
  ['subscribers.default_max_connections rejects zero', loadSubscriberConfig, ['subscribers', 'default_max_connections'], '0', /subscribers\.default_max_connections/, /minst 1/],
  ['subscribers.default_max_connections rejects negative values', loadSubscriberConfig, ['subscribers', 'default_max_connections'], '-1', /subscribers\.default_max_connections/, /minst 1/],
  ['mqtt.json_publish_max_bytes rejects fractional values', loadMqttConfig, ['mqtt', 'json_publish_max_bytes'], '12.5', /mqtt\.json_publish_max_bytes/, /heltal/],
  ['broker.node_name_cache_ttl_ms rejects non-positive values', loadMqttConfig, ['broker', 'node_name_cache_ttl_ms'], '0', /broker\.node_name_cache_ttl_ms/, /större än 0/],
  ['broker.kv_url is required', loadMqttConfig, ['broker', 'kv_url'], undefined, /broker\.kv_url/, /saknas/],
])('%s before startup', (_name, loadConfig, path, value, configPattern, messagePattern) => {
  const document = baseConfig();
  if (value === undefined) {
    deletePath(document, path);
  } else {
    path.slice(0, -1).reduce((current, part) => current[part], document)[path[path.length - 1]] = value;
  }
