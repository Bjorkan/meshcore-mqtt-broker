...
- The broker is stateless: MQTT sessions, retained packets, queues, wills, and upload queues live only in process memory. `/data/meshcore-mqtt-broker/` holds the broker instance id and health credentials.
- The broker has no dashboard, REST API, OpenAPI, MCP, or browser frontend HTTP surface beyond the unauthenticated `GET /status` (`{ status: "ok", storage: "stateless" }`). MQTT subscriber roles apply only to MQTT.
