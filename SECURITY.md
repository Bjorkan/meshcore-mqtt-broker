...
- Every MCP result is recursively checked immediately before serialization. Sensitive field names are blocked; e-mail addresses, IP addresses, credentials, internal topics, and sensitive paths are redacted at any nesting level; unsupported or excessively complex output fails closed. Protocol errors do not expose SQL, paths, stack traces, or exception details. Keep this policy and its adversarial tests in place when adding tools.
