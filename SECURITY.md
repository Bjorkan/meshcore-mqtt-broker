...
- Every MCP and `/api/v2/tools/*` result is recursively checked immediately before serialization by the same policy. Sensitive field names are blocked; e-mail addresses, IP addresses, credentials, internal topics, and sensitive paths are redacted at any nesting level; unsupported, excessively complex, or over-4-MiB output fails closed. Transport errors do not expose SQL, paths, stack traces, or exception details. Keep the shared registry, policy, and adversarial parity tests in place when adding tools.
