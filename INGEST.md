...
On startup, stale `processing` claims return to `pending`; failed events return only when they have no recorded processing errors, so poison payloads are not re-processed on every boot. Normalized writes occur in transactions after the raw receipt already exists. The terminal status update is part of that same transaction, and `fail()` only applies to rows still marked `processing`, so a committed success can never be re-marked `failed` by a late error path. Event-owned normalized rows are replaced before a retry, unique keys fence identity races, observer IATA aggregates are maintained incrementally and recomputed only around a retry, and raw retention expires only finalized `processed`/`processed_with_warnings` rows. Before normalized facts are written, the worker creates/refreshes compact `mqtt_event_provenance`; those facts therefore survive raw-payload expiry. `pending`, `processing`, and `failed` events remain in the raw journal. This makes retry, restart, and retention idempotent.
...
`reprocessMqttEvents()` supports time, observer, subtopic, processing status, parser version, failed-only, bounded limit, and cursor filters. `reprocessPackets()` supports time, observer, decode status, decoder version, failed-only, and bounded limit filters. Reprocessing never changes `received_at_ms` and therefore never extends either raw or normalized retention. Once a raw event has expired it is no longer replayable; normalized facts and compact provenance may still be retained according to their independent policy.
